...
const { hardwareProjects, softwareProjects, openSourceCommunities } =
  projects;

const { securityCerts, platformCerts, otherCerts } = certifications; 


/**
 * Creates and populates a list of talks with specified properties
 *
 * @function
 *
 * @param {Array} items - An array of objects, each representing a talk
 * @param {string} id - The ID of the parent element where the list of talks will be appended
 *
 * @returns {undefined}
 */

function populateTalks(items, id) {
  const talkDesign = document.getElementById(id);

  for (let i = 0; i < items.length; i++) {
    const h4 = document.createElement("h4");
    h4.className = "talk-heading";
    ...
    divProjectCard.className = "talk-card";
...
function populateCertifications(items, id) {
  let certificationdesign = document.getElementById(id);

  let h4 = document.createElement("h4");
  h4.className = "certification-heading";
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    divSpan.innerHTML = "";
    for (let k = 0; k < items[i].areas.length; k++) {
      let span = document.createElement("span");
      span.className = "badge badge-secondary";
      span.innerHTML = items[i].areas[k];
      divSpan.append(span);
    }

    certificationdesign.append(li.cloneNode(true));

    if (i != items.length - 1) {
      certificationdesign.append(hr.cloneNode(true));
    }
  }
}
